...
module FedaPay
  ...
  \"object\": \"event\"
    ...
    def generateHeader(opts = {})
      ...
      opts[:signature] ||= FedaPay::WebhookSignature.compute_signature(
        opts[:timestamp],
        opts[:payload],
        opts[:secret]
      )
      FedaPay::WebhookSignature.generate_header(
        opts[:timestamp],
        opts[:signature],
        scheme: opts[:scheme]
      )
    ...
    describe "compute signature" do
      it "compute a signature which can then be verified" do
        timestamp = Time.now
        signature = FedaPay::WebhookSignature.compute_signature(
          EVENT_PAYLOAD,
          SECRET
        )
    ...
    describe "generate header" do
      before "generate a header in valid format" do
        ...
        scheme = "v1"
        header = FedaPay::WebhookSignature.generate_header(
          timestamp,
          signature,
          scheme: scheme
        )
    ...
    describe "construct event" do
      it "return an Event instance from a valid JSON payload and valid signature header" do
        ...
        event = FedaPay::Webhook.construct_event(EVENT_PAYLOAD, header, SECRET)
      ...
      it "raise a JSON::ParserError from an invalid JSON payload" do
        assert_raises JSON::ParserError do
          ...
          FedaPay::Webhook.construct_event(payload, header, SECRET)
        end
      end
      ...
      it "raise a Signature Verification Error from a valid JSON payload and an invalid signature header" do
        ...
        assert_raises FedaPay::SignatureVerificationError do
          FedaPay::Webhook.construct_event(EVENT_PAYLOAD, header, SECRET)
        end
      end
    end
    ...
    describe "verify signature header" do
      it "raise a SignatureVerificationError when the header does not have the expected format" do
        ...
        assert_raises(FedaPay::SignatureVerificationError) do
          FedaPay::WebhookSignature.verify_header(EVENT_PAYLOAD, header, "secret")
        end
      end
      ...
      it "raise a Signature verification error when there are no valid signatures for the payload" do
        header = generateHeader(signature: "bad_signature")
        assert_raises(FedaPay::SignatureVerificationError) do
          FedaPay::WebhookSignature.verify_header(EVENT_PAYLOAD, header, "secret")
        end
      end
      ...
      it "raise a Signature verification error when the timestamp is not within the tolerance" do
        ...
        assert_raises(FedaPay::SignatureVerificationError) do
          FedaPay::WebhookSignature.verify_header(EVENT_PAYLOAD, header, SECRET, tolerance: 10)
        end
      end

      it "return true when the header contains a valid signature and the timestamp is within the tolerance" do
        header = generate_header
        assert(FedaPay::WebhookSignature.verify_header(EVENT_PAYLOAD, header, SECRET, tolerance: 10))
      end
      ...
      it "return true when the header contains a valid signature and the timestamp is off but no tolerance is provided" do
        ...
        assert(FedaPay::WebhookSignature.verify_header(EVENT_PAYLOAD, header, SECRET))
      end
    end
  end
end

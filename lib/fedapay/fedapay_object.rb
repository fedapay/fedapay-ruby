...
module FedaPay
  class FedaPayObject
    ...
    def self.construct_from(values, opts = {})
      values = FedaPay::Util.symbolize_names(values)

      # work around protected #initialize_from for now
      new(values[:id]).send(:initialize_from, values, opts)
    end

    # Determines the equality of two FedaPay objects. FedaPay objects are
    # considered to be equal if they have the same set of values and each one
    # of those values is the same.
    def ==(other)
      other.is_a?(FedaPayObject) && @values == other.instance_variable_get(:@values)
    end

    # Hash equality. As with `#==`, we consider two equivalent FedaPay objects equal.
    def eql?(other)
      # Defer to the implementation on `#==`.
      self == other
    end

    # As with equality in `#==` and `#eql?`, we hash two FedaPay objects to the
    ...
    #   which applies only to new FedaPayObjects being initiated under this
    #   FedaPayObject. Defaults to true.
    ...
    # Sets all keys within the FedaPayObject as unsaved so that they will be
    # included with an update when #serialize_params is called. This method is
    # also recursive, so any FedaPayObjects contained as values or which are
    ...
    # serialized.
    def marshal_dump
      # The FedaPayClient instance in @opts is not serializable and is not
      # really a property of the FedaPayObject, so we exclude it when
      # dumping
      opts = @opts.clone
      opts.delete(:client)
      [@values, opts]
    ...
    def serialize_params(options = {})
      ...
      @values.each do |k, v|
        ...
        #   3. Its value is a FedaPayObject. A FedaPayObject may contain modified
        #      values within in that its parent FedaPayObject doesn't know about.
        #
    ...
    def method_missing(name, *args)
      ...
      begin
        ...
        raise NoMethodError, e.message + ".  HINT: The '#{name}' attribute was set in the past, however.  It was then wiped when refreshing the object with the result returned by FedaPay's API, probably as a result of a save().  The attributes currently available on this object are: #{@values.keys.join(', ')}"
      end
    end
    ...
    # * +:opts:+ Options for FedaPayObject like an API key.
    ...
    def serialize_params_value(value, original, unsaved, force, key: nil)
      ...
      # making sure any time one is set, we convert it to a FedaPayObject. This
      ...
      # existing array being held by a FedaPayObject. This could happen for
